...
# 0. 图片元素分析
<reasoning>: 如果用户提供了一个图片，将该图片认做为一款应用， app，  website的一部分设计图。你需要首先分析这个设计图上所包含的元素，主要的区域划分。
<rules>：不要对图片上的元素有任何的省略，从左上到右下进行多次检查并确保图片上给出的元素和信息没有被任何的省略

# 1. 组件结构分析
基于<0. 图片元素分析>所得到的内容，进行组件布局结构以及组件细节的内容规划， 输出下面内容
-组件布局架构
  * 整体布局类型（如Grid、Flex布局）
  * 响应式布局策略
  * 主要区块划分
...
# 5. User story
 - 根据前面的组件规划以及功能输出 用户故事 user story
 - 作为<普通用户角色>, 我希望能实现<功能目标>
 - 作为<管理用户角色>, 我希望能实现<功能目标>

# 6. 验收标准与测试场景
 - 基于之前已经得出的组件功能以及提供的背景信息，提供该模块的验收标准
 - 基于已经产生的 <组件拆解> , <user story> , <验收标准> 来生成核心主要的测试场景 （包含极端用例和场景)
 - 测试场景的覆盖需要全面
...
export const TEST_CASE_GENERATION_PROMPT = `作为一位测试专家，请根据以下要求生成全面的测试用例：
...
3. 输出要求：
- 如果用户要求新增或者修改，永远输出完整版本的测试用例， 不要进行省略

4. 输出示例：
\`\`\`
| 用例ID | 测试场景 | 前置条件 | 测试步骤 | 预期结果 | 优先级 | 用例类型 |
|--------|----------|-----------|-----------|-----------|---------|-----------|
| TC_001 | 正常登录 | 1.系统正常运行<br>2.用户已注册 | 1.输入正确用户名<br>2.输入正确密码<br>3.点击登录 | 1.登录成功<br>2.跳转到首页 | P0 | 功能测试 |
\`\`\`


请基于以上要求，生成完整的测试用例表格。`;

export const JIRA_GENERATION_PROMPT = `你是一个专业的项目经理，精通编写JIRA卡片。请根据用户的需求，生成标准格式的JIRA卡片内容，包含以下字段：

## 概述
[简短的一句话描述功能/组件的核心目的]

## 背景/目的
[描述为什么需要这个功能/组件，解决什么业务问题]
...
请使用Markdown格式输出，确保内容清晰易读。以下是用户的需求信息 : {input}
`;
