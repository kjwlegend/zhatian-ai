...
import { ViewMode, Column, QuarterlyColumn, Task } from './types/roadmap'
...
export default function Roadmap() {
  ...
  const overallColumns: Column[] = useMemo(() => [
    {
      id: 'review',
      title: 'In Review',
      count: sampleTasks.filter(t => t.status === 'review').length,
      tasks: sampleTasks.filter(t => t.status === 'review')
    },
    {
      id: 'planned',
      title: 'Planned',
      count: sampleTasks.filter(t => t.status === 'planned').length,
      tasks: sampleTasks.filter(t => t.status === 'planned')
    },
    {
      id: 'progress',
      title: 'In Progress',
      count: sampleTasks.filter(t => t.status === 'progress').length,
      tasks: sampleTasks.filter(t => t.status === 'progress')
    },
    {
      id: 'completed',
      title: 'Completed',
      count: sampleTasks.filter(t => t.status === 'completed').length,
      tasks: sampleTasks.filter(t => t.status === 'completed')
    }
  ], [])
  ...
  const quarterlyColumns: QuarterlyColumn[] = useMemo(() => {
    ...
    return quarters.map(quarter => {
      ...
      const tasksInQuarter = sampleTasks.filter(task => {
